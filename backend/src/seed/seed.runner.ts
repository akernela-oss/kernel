...
export async function seedDatabase(
  ...
  for (const c of [
    ...
    const { old, ...data } = c;
    void old;
    const row = await prisma.customer.create({ data: { ...data, ownerId: sellerId } });
  ...
  for (const s of [
    {
      ...
      quantity: 2,
      marketPrice: 93_000_000,
      discount: 0.14,
      contractNo: 'CON-1001',
      contractStatus: 'امضا شده',
      deliveryDate: '2026-06-24',
      orderStatus: 'ثبت شده',
      nextAction: 'تماس با تامین‌کننده',
    },
    {
      old: 'ORD-1002',
      customer: 'CUS-002',
      ...
      quantity: 1,
      marketPrice: 76_000_000,
      discount: 0.07,
      contractNo: 'CON-1002',
      contractStatus: 'در انتظار امضا',
      deliveryDate: '2026-06-20',
      orderStatus: 'ثبت شده',
      nextAction: 'تکمیل قرارداد',
    },
    {
      old: 'ORD-1003',
      customer: 'CUS-003',
      ...
      quantity: 1,
      marketPrice: 69_000_000,
      discount: 0.25,
      contractNo: 'CON-1003',
      contractStatus: 'ناقص',
      deliveryDate: '2026-06-18',
      orderStatus: 'ثبت شده',
      nextAction: 'پیگیری چک',
    },
  ]) {
    const row = await prisma.sale.create({
      data: {
        customerId: customerMap.get(s.customer) as string,
        memberId: memberMap.get(s.member) as string,
        date: d(s.date),
        category: s.category,
        model: s.model,
        quantity: s.quantity,
  ...
  for (const dbt of [
    {
      old: 'DEBT-001',
      creditor: 'تامین‌کننده A',
      type: 'تامین‌کننده',
      description: 'خرید عمده آیفون',
  ...
  for (const inv of [
    {
      old: 'INV-001',
      title: 'سرمایه‌گذاری کوتاه‌مدت تامین کالا',
      source: 'ACC-04',
      startDate: '2026-06-10',
      currentValue: 56_000_000,
      status: 'فعال',
      exitTarget: '2026-07-10',
    },
    {
      old: 'INV-002',
      title: 'ذخیره سود عملیاتی',
      source: 'ACC-04',
      startDate: '2026-06-12',
      currentValue: 0,
      status: 'در انتظار تصمیم',
      exitTarget: '2026-08-01',
    },
  ]) {
  ...
  for (const t of [
    ...
    },
    {
      ...
      account: 'ACC-01',
      type: 'سرمایه‌گذاری',
      party: 'صندوق سرمایه‌گذاری داخلی',
      investment: 'INV-001',
      inflow: 0,
      outflow: 50_000_000,
      method: 'حواله بانکی',
      note: 'سرمایه‌گذاری نقدی',
    },
    {
      old: 'TRX-1008',
      date: '2026-06-11',
      ...
      outflow: 25_000_000,
      method: 'حواله بانکی',
      note: 'تسویه بخشی بدهی تامین‌کننده',
    },
  ]) {
    const row = await prisma.transaction.create({
      data: {
        date: d(t.date),
        accountId: accountMap.get(t.account) as string,
        type: t.type,
        party: t.party,
  ...
  await prisma.check.update({
    where: { id: checkMap.get('CHK-1001') as string },
    data: { transactionId: txMap.get('TRX-1002') as string },
  });
    ...
    data: [
      {
        title: 'اجاره دفتر',
        category: 'اداری',
        monthlyAmount: 30_000_000,
        dueDay: 10,
        owner: 'مدیر مالی',
        status: 'فعال',
      },
      {
        title: 'حقوق تیم پشتیبانی',
        category: 'حقوق',
        monthlyAmount: 85_000_000,
        dueDay: 28,
        owner: 'مدیر مالی',
        status: 'فعال',
      },
      {
        title: 'اینترنت و نرم‌افزارها',
        category: 'زیرساخت',
...
function defaultKpis() {
  return [
    ...
    },
    {
      ...
      unit: 'تومان',
      warning: 260_000_000,
      critical: 350_000_000,
      visibility: 'admin,finance',
    },
    {
      title: 'تعداد سفارش فعال',
      group: 'فروش',
      metric: 'activeOrders',
      target: 20,
      direction: 'higher',
      ...
      warning: 10,
      critical: 5,
      visibility: 'admin,finance,seller',
    },
    {
      title: 'میانگین تخفیف',
      group: 'فروش',
    ...
    {
      title: 'قراردادهای ناقص',
      group: 'قرارداد',
      metric: 'incompleteContracts',
      target: 0,
      direction: 'lower',
      unit: 'عدد',
      warning: 2,
      critical: 5,
      visibility: 'admin,finance,contract',
    },
    {
      title: 'سفارش‌های بحرانی',
      group: 'عملیات',
      metric: 'criticalOrders',
      target: 0,
      direction: 'lower',
      unit: 'عدد',
      warning: 2,
    ...
    {
      title: 'ROI سرمایه‌گذاری',
      group: 'سرمایه‌گذاری',
      metric: 'investmentROI',
      target: 10,
      direction: 'higher',
      unit: '٪',
      warning: 3,
